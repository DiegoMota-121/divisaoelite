<?php
    ...
    if(!($usuario->senha == $usuario->confirmar_senha)){
        header('Location: cadastro_professor.php?cadastro=error1');
        die;
    }
    ...
    if(strlen($usuario->senha) < 9){
        header('Location: cadastro_professor.php?cadastro=error2');
        die;
    }

    /*verifica se o email digitado já existe no banco*/

    $sql_email = "SELECT email FROM professor WHERE email = '{$usuario->email}'";
    $email_existentes = $conn->query($sql_email);

    if($email_existentes->num_rows > 0){
        header('Location: cadastro_professor.php?cadastro=erro3');
        die;
    }
    ...
    $sql = "INSERT INTO professor (nome, email, senha) VALUES ('{$usuario->nome}','{$usuario->email}','{$senha_criptografada}')";
    $usuario_sql = mysqli_query($conn, $sql);
    
    if($usuario_sql){
        header('Location: login_professor.php?cadastro=success');
        die;
    } else {
        header('Location: login_professor.php?cadastro=error4');      
        die;
    }
