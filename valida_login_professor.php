...
$sql_senha = "SELECT senha FROM professor WHERE senha = '" . $usuario->senha."'";
$sql_senha_selecionado = $conn->query($sql_senha) or die($mysqli->error);
$senha = $sql_senha_selecionado->fetch_array();

if(!isset($email)){
    //email não encontrado
    header('Location: login_professor.php?login=error1');
    die;
}
if(!isset($senha)){
...
if ($usuario_autenticado) {
    ...
    header('Location: login_aluno.php?login=error2');
}
